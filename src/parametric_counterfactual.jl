...
"""
Base.@kwdef mutable struct ParametricCounterfactualDyadicKernelDensityEstimator
    ...
    X0::Vector{Int}
    X1::Vector{Int}
    phat0::Vector{Float64}
    phat1::Vector{Float64}
    meta::Dict
...
"""
function ParametricCounterfactualDyadicKernelDensityEstimator(kernel_name::String,
                                                              bandwidth::Float64,
                                                              significance_level::Float64,
                                                              n_resample::Int,
                                                              sdp_solver::String,
    ...
    est = ParametricCounterfactualDyadicKernelDensityEstimator(
                                                               # input parameters
                                                               kernel_name,
                                                               bandwidth,
                                                               significance_level,
...
function estimate_kappahat(est::ParametricCounterfactualDyadicKernelDensityEstimator)
    ...
    for r in 1:(est.n_data)
        for i in 1:(est.n_data)
            term1[r, i] = ((est.X0[r] == est.X1[i]) - est.phat0[est.X1[i]]) / est.phat1[est.X1[i]]
            term2[r, i] = (est.phat0[est.X1[i]] / est.phat1[est.X1[i]]) *
                          ((est.X1[r] == est.X1[i]) - est.phat1[est.X1[i]]) / est.phat1[est.X1[i]]
        end
    end
